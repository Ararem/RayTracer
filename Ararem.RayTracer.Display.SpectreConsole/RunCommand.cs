...
#region Markup Styles
	...
	private static (Scene Scene, RenderOptions Options) ConfirmSettings(CommandContext context, Settings settings)
	{
		RenderOptions renderOptions = new()
		{
				...
				KMin               = settings.KMin,
				KMax               = settings.KMax,
				ConcurrencyLevel   = settings.Concurrency,
				Passes             = settings.Passes,
				MaxBounceDepth     = settings.MaxDepth,
				DebugVisualisation = settings.DebugVisualisation
		};
		...
		//Select scene
		Scene scene = Prompt(new SelectionPrompt<Scene>().Title($"[{TitleMarkup}]Please select which scene you wish to load:[/]").AddChoices(BuiltinScenes.GetAll()).UseConverter(s => $"[{SceneMarkup}]{s}[/]"));
	...
	private sealed record LiveDisplayClosure(string AppTitle, AsyncRenderJob RenderJob, int Interval)
	{
		...
		internal async Task LiveDisplayAsync(LiveDisplayContext ctx)
		{
			...
			statsAndImageTable.AddColumns(new TableColumn($"[{HeadingMarkup}]Render Statistics[/]\n").Centered(), new TableColumn($"[{HeadingMarkup}]Image Preview[/]\n").Centered());
	...
	#region Render stats table
		...
		renderStatsTable.AddRow("",                                        $"{options.RenderWidth.ToString(numFormat),numAlign}          pixels wide");
		renderStatsTable.AddRow("",                                        $"{options.RenderHeight.ToString(numFormat),numAlign}          pixels high");
		renderStatsTable.AddRow("",                                        "");
		...
		for (int i = 0; i < depths.Count; i++)
			chart.AddItem(
					$"[[{depths[i].range}]]", depths[i].count / renderJob.RenderStats.RawRayDepthCounts.Sum(u => (double)u), //https://www.desmos.com/calculator/erite0if8u
					Color.White
			);
		#endif

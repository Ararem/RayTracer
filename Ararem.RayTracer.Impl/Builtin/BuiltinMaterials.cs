...
public static class BuiltinMaterials
{
	...
	{
		return typeof(BuiltinMaterials).GetProperties(BindingFlags.Public | BindingFlags.Static).Where(p => p.PropertyType == typeof(Material)).Select(p => (Material)p.GetValue(null)!);
	}
}
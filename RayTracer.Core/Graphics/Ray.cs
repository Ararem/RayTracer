...
public readonly record struct Ray(Vector3 Origin, Vector3 Direction)
{
	...
	/// </summary>
	public static Ray FromPoints(Vector3 p1, Vector3 p2) => new Ray(p1, Vector3.Normalize(p2 - p1));

	/// <summary>
	/// Returns a copy of the current ray with the <see cref="Direction"/> vector normalized
	/// </summary>
	public Ray WithNormalizedDirection() => new Ray(Origin, Vector3.Normalize(Direction));
}
	...
	public static Scene Demo
	{
		get
		{
			...
			}
			{
				...
				objects.Add(new SceneObject("Capsule", new Capsule(new Vector3(-2, .7f, -3), new Vector3(0, 1.5f, -1f), .7f), new RefractiveMaterial(RefractiveMaterial.GlassIndex, new Colour(0.27058825F, 0.77254903F, 1F))));
			}
		...
		get
		{
			...
			return new Scene(
					"Cornell Box", Camera.Create(new Vector3(278, 278, -800), new Vector3(278, 278, 0), UnitY, 40f, 1f / 1f, 0f, 1f), new SceneObject[]
					{
							...
							new ("Small Box Sphere", new Sphere(new Vector3(212.5f, 265f, 147.5f), 100), new EmissiveRefractiveMaterial(RefractiveMaterial.GlassIndex, White, Blue, true)),

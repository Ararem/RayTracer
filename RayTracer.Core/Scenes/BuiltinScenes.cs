...
public static class BuiltinScenes
{
	...
	public static readonly Scene ThreeSpheres = new(
			"Three Spheres", new Camera(new Vector3(0, 1, 5), Vector3.Zero, Vector3.UnitY, 20, 16f / 9f), new SceneObject[]
			{
	...
	public static readonly Scene WithGround = new(
			"With Ground", new Camera(new Vector3(0, 1, -5), Vector3.Zero, Vector3.UnitY, 20, 16f / 9f), new SceneObject[]
			{
					new("Sphere", new Sphere { Radius = 1f, Centre    = new Vector3(0, 1f, 0) }, new DiffuseMaterial(Colour.Green)),
					new("Ground", new Sphere { Radius = 1000f, Centre = -1000f * Vector3.UnitY }, new DiffuseMaterial(0.5f * Colour.White))
			},
			new DefaultSkyBox()
	);
}
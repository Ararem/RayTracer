...
/// <summary>
/// Static class that contains definitions for built-in styles. To reference a style, you should use it's name: <c>myControl.Style = nameof(Styles.Bold);</c>
/// </summary>
/// <remarks>
/// Most styles should work for most widgets, but if a style isn't working when it should (e.g. <see cref="Heading"/> on a <see cref="GroupBox"/>) due to mismatched base types, you can try using the `Force_XXX` style instead.
/// This will attempt to dynamically access the relevant members of the type to assign them. This will only work in specific scenarios, where the target type (i.e. <see cref="GroupBox"/>) has a definition for a property that should be styled (i.e. <see cref="GroupBox.Font"/>), but the standard style (i.e. <see cref="Heading"/>) only works for a different base type (i.e. <see cref="CommonControl"/>). In this example, since the <see cref="GroupBox"/> has a "Font" property, we can use the <see cref="Force_Heading"/> style to dynamically (forcefully) change the property (since it accepts any object of type <see cref="Widget"/>).
/// </remarks>
public static class Styles
{
	...
	public static StyleWidgetHandler<CommonControl> Heading => static control => { control.Font = new Font(FontFamilies.Sans, HeadingFontSize); };
	...
	public static StyleWidgetHandler<Widget> Force_Heading => static widget => { ((dynamic)widget).Font = new Font(FontFamilies.Sans, HeadingFontSize); };
	...
	internal static void RegisterStyles()
	{
		...
		foreach (PropertyInfo prop in typeof(Styles).GetProperties(BindingFlags.Public | BindingFlags.Static).Where(p => p.PropertyType.IsGenericType && (p.PropertyType.GetGenericTypeDefinition() == typeof(StyleWidgetHandler<>))))
		{

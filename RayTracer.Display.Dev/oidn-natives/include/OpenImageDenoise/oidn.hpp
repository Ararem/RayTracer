...
OIDN_NAMESPACE_BEGIN
    ...
    enum class Access
    {
        Read = OIDN_ACCESS_READ,
        // read-only access
        Write = OIDN_ACCESS_WRITE,
        // write-only access
        ReadWrite = OIDN_ACCESS_READ_WRITE,
        // read and write access
        WriteDiscard = OIDN_ACCESS_WRITE_DISCARD,
    ...
    using ProgressMonitorFunction = bool(*)(void* userPtr, double n);
    ...
    public:
                      ...
                      size_t bytePixelStride = 0, size_t byteRowStride = 0)
        {
            oidnSetSharedFilterImage(handle, name,
                                     ptr, static_cast<OIDNFormat>(format),
                                     width, height,
                                     byteOffset,
                                     bytePixelStride, byteRowStride);
        }
    ...
    enum class Error
    {
        ...
        Cancelled = OIDN_ERROR_CANCELLED,
        // the operation was cancelled by the user
    };

    // Error callback function
    using ErrorFunction = void(*)(void* userPtr, Error code, const char* message);

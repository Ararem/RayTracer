...
public abstract class SimpleLightBase : Light
{
	...
	///  however, the light <i>should</i> be significantly attenuated after this point to the point where it is not noticeable. For certain functions (such
	///  as <see cref="LinearDistanceAttenuation"/>, it is necessary to set the <see cref="CutoffRadius"/> as well, since the function goes below zero)
	/// </remarks>
	public float AttenuationRadius { get; init; } = 1f;

	/// <summary>
	///  The radius at which the light does not illuminate anymore. After this distance, the light will be forcibly clamped to
	///  <see cref="RayTracer.Core.Colour.Black"/>. This will be necessary to set for <see cref="DistanceAttenuationFunc">attenuation delegates</see> that go
	///  below zero or only work in the range [0..<see cref="AttenuationRadius"/>]
	/// </summary>
	...
	/// <returns>
	///  A tuple containing a <see cref="Ray"/> and two <c>k</c> values that are the (distance) bounds along the ray. If there is an intersection in the
	///  scene, and the <see cref="HitRecord.K"/> value is &gt; kMin and &lt; kMax, the hit is considered shadowed. The <c>kMax</c> value should be equal to
	///  the distance between the point on the hit object, and the closest intersection between the surface of the light source and the ray.
	/// </returns>
	[PublicAPI]
...
#region Attenuation things
	...
	///  Steepness of the curve. Note that this should be <i>positive</i> despite the fact that the curve is an inverted logistics curve (it is translated
	///  negatively internally)
	/// </param>
	...
	/// <summary>Attenuation decays exponentially with distance; <c>y=e^(-ax)</c></summary>
	...
	/// <summary>
	///  Delegate used to calculate how much the intensity of the light should be attenuated at a given <paramref name="normalisedDistance"/>
	/// </summary>
